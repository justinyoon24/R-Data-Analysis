...
n = 100
x = 25
dnorm(25, 0.2*100, sqrt(0.2*100*(1-0.2)))
dbinom(25, 100, 0.2)
pbinom(25.5, 100, 0.2) - pbinom(24.5, 100, 0.2)
```
...
```{r test1aiv}
pnorm(25, 0.2*100, sqrt(0.2*100*(1-0.2)), lower.tail = TRUE)
pbinom(25, 100, 0.2)
...
```{r test1b}
dp <- (dpois(0:10,lambda = 0.1*100))
db <- (dbinom(0:10, 100, 0.1))
par(mfrow = c(1,2))
barplot(dp, main = "Poisson Probability", col = 'blue', xlab = "Outcome", ylab = "Probability")
barplot(db, main = "Binomial Probability", col = 'red', xlab = "Outcome", ylab = "Probability")
...
expx <- round(sum(x*prob), digits = 1)
expx
varx <- round(((sum((x^2)*prob))-(expx^2)),digits = 1)
varx
...
cumprob <- cumsum(prob)
cumprob
plot(x, cumprob, type = "s", main = "Cumulative Probabilities vs Corresponding Outcomes", xlab = "Outcomes", ylab = "Cumulative Probability")
text(3, .92, "Median = 0.867")
```
...
hist(faithful$waiting, main = "Frequency of Waiting Times", xlab = "Waiting Time", col = "Green")
wait <- subset(faithful, waiting > 70)
erupt <- subset(faithful, eruptions < 3.5)
condprob <- nrow(erupt)/nrow(wait)
condprob
```
...
```{r test2ai}
plot(faithful$waiting, faithful$eruptions, main = "Faithful Eruption Durations vs Waiting Time", xlab = "Waiting Time", ylab = "Eruption Duration")
a <- subset(faithful, waiting > 90 & eruptions > 5)
a
abline(h=5,v=90,col="blue")
points(96, 5.1, col= "red", pch = 20)
   
```

(ii)  What does the plot suggest about the relationship between eruption time and waiting time?

***Answer: There appears to be a positive correlation between eruption time and waiting time. As waiting time increases, eruption time increases as well.    ***
...
mwait
plot(x = mwait[,1],y = mwait[,2], xlab = "First Waiting Time", ylab = "Second Waiting Time", main = "First Column Waiting Time vs Second Column Waiting Time")
```
...
```{r test2c}
cor.test(mwait[,1], mwait[,2],alternative = "two.sided", method = "kendall", conf.level = .99)
...
boxplot(Diet1, main = "Chick Weights in Diet 1", ylab = "Chick Weight", col = "green")
boxplot(Diet3, main = "Chick Weights in Diet 3", ylab = "Chick Weight", col = "orange")
...
```{r test3b}
t.test(Diet1, Diet3, alternative = "two.sided", conf.level = .95)
...
before <- subset(ChickWeight[index,], Time == 20, select = weight)$weight
after <- subset(ChickWeight[index,], Time == 21, select = weight)$weight
cbind(before,after)
...
```{r test3c}
plot(before,after, main = "Day 20 vs Day 21",xlab = "Day 20", ylab = "Day 21")
abline(0,1,col = "red")
...
conf <- mean_diff - crit_val*(sd/sqrt(n))
cat("One-Sided Confidence Interval: (", conf, ", oo)\n")
...
boxplot(Nile, col = "green", main = "Boxplot of Nile River", ylab = "Sample Quantiles")
...
curve(dnorm(x, mean(sample1), sd(sample1)), add = TRUE, col = "red", lty =1, lwd =2)
hist(sample2, freq = FALSE, xlim = c(750,1050), ylim = c(0, 0.025), col = "yellow")
curve(dnorm(x, mean(sample2), sd(sample2)), add = TRUE, col = "red", lty =1, lwd =2)
...
median_breaks
hist(warpbreaks$breaks, col = "grey", main = "Number of Breaks", xlab = "Number of Breaks")
...
summary(warpbreaks2)
contingency <- table(warpbreaks2$wool, warpbreaks2$number)
contingency
```
...
#Based on the p-value being greater than the significance level, we must fail to reject the null hypothesis. There is not sufficient evidence to suggest that the variables are not independent. 
```
...
tbl <- table(warpbreaks2$wool, warpbreaks2$number)
...
chisq_function <- function(x) {
  ...
  return(list("chi-squared" = chi_sq, "degrees of freedom" = df, "p-value" = pval))
